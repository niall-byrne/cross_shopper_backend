...
class TestMemoizeDecorator:
  ...
  def test_memoized_method__same_instances__same_args__shares_cache(
      self,
  ...
  def test_memoized_method__same_instances__diff_args__isolated_cache(
      self,
      mocked_serializer: MockSerializer,
      mocked_arg: str,
  ) -> None:
  ...
  def test_memoized_method__diff_instances__same_repr__shares_cache(
      self,
      mocked_serializer: MockSerializer,
  ...
  def test_memoized_method__diff_instances__diff_repr__isolated_cache(
      self,
      mocked_serializer: MockSerializer,
  ...
  def test_memoized_func__none_result__returns_cached_none(self) -> None:
    call_count = 0

    @memoize()
    def return_none(_x: Any) -> None:
      nonlocal call_count
      call_count += 1
      return None
  ...
  def test_memoized_func__same_func__same_args__shares_cache(self) -> None:
    call_counts = {"f1": 0}
  ...
  def test_memoized_func__vary_func__same_args__isolated_caches(self) -> None:
  ...
  def test_memoized_func__timeout__passes_timeout_to_backend(self) -> None:
    timeout_value = 123

    @memoize(timeout=timeout_value)
    def timed_func(x: Any) -> Any:
      return x
  ...
  def test_memoized_func__vary_args__returns_correct_cached_results(
      self,
      val: Any,
  ) -> None:
    call_count = 0
