...
class TestItemSerializer:
  ...
  def test_deserialization__valid_input__existing_scraper__correct_model(
    ...
    assert instance.packaging.container.name == item_data["packaging"][
        "container"]
    ...
    assert instance.scraper_config.all(
    )[0].url == item_data["scraper_config"][0]["url"]
    assert instance.scraper_config.all()[1].scraper == scraper
    assert instance.scraper_config.all(
    )[1].url == item_data["scraper_config"][1]["url"]
