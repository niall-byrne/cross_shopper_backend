...
class TestGenericListFilter:
  ...
  def test_lookups__not_boolean__vary_reverse__returns_correct_lookups(
    ...
    mocked_qs.values_list.return_value.distinct.return_value.order_by.return_value = [
        "a",
        "b",
    ]
    ...
    mocked_qs.values_list.return_value.distinct.return_value.order_by.assert_called_once_with(
        f"{operator}test_param"
    )
