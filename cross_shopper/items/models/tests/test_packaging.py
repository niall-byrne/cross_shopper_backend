...
class TestPackaging:
    ...
    assert str(exc.value) == str(
        {
            "__all__":
                [
                    "Packaging with this Quantity, Container and Unit already exists.",
                ]
        }
    )
  ...
  def test_str__with_container__with_quantity__with_unit__correct_value(
    ...
    assert str(packaging) == (
        f"{str(packaging.container)}: {str(packaging.quantity)} {str(packaging.unit)}"
    )
  ...
  def test_str__no_container__no_quantity__with_unit__correct_value(
    ...
    assert str(packaging) == (
        f"{packaging.CONTAINER_NAME_FOR_BULK_PACKAGING} per {str(packaging.unit)}"
    )
