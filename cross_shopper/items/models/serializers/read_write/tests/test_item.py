...
class TestItemSerializerRW:
  ...
  def compare_instance_to_data(
    ...
    assert item_instance.packaging.container.name == item_data["packaging"][
        "container"]

    assert item_instance.price_group is not None
    assert item_instance.price_group.name == item_data["price_group"]["name"]
    assert (
        item_instance.price_group.is_non_gmo == item_data["price_group"]
        ["is_organic"]
    )
    assert (
        item_instance.price_group.is_organic == item_data["price_group"]
        ["is_organic"]
    )
    assert item_instance.price_group.quantity == item_data["price_group"][
        "quantity"]
    assert item_instance.price_group.unit.name == item_data["price_group"][
        "unit"]
