...
class Price(
  ...
  @cached_property
  def last_52_weeks_high(self) -> Optional[decimal.Decimal]:
  ...
  @cached_property
  def last_52_weeks_low(self) -> Optional[decimal.Decimal]:
    """Calculate the lowest item price for the past 52 weeks at this store."""
    if self._last_52_weeks.exists():
      return self._last_52_weeks.\
          aggregate(Min('amount'))["amount__min"].\
          quantize(decimal.Decimal('.00'))
